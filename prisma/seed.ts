    ...
    for (let i = 0; i < 2; i++) {
      ...
      const client = await prisma.client.create({
        data: {
          ...
          admissionDate,
          houseId: house.id,
          mhCaseManagerName: `MH CM ${lastName}`,
          mhCaseManagerEmail: `mh.cm.${lastName.toLowerCase()}@county.gov`,
          mhCaseManagerPhone: `(612) 555-${String(Math.floor(Math.random() * 9000) + 1000)}`,
          cadiCaseManagerName: `CADI CM ${lastName}`,
          cadiCaseManagerEmail: `cadi.cm.${lastName.toLowerCase()}@county.gov`,
          cadiCaseManagerPhone: `(612) 555-${String(Math.floor(Math.random() * 9000) + 1000)}`,
